...
import {
  REGEXP,
  DECLARATION_REGEXP,
} from '../../../lib/variables/strategies/css-strategy';
import CssExtractor from '../../../lib/variables/strategies/css-strategy';
import VariablesManager from '../../../lib/variables/variables-manager';
import type Variable from '../../../lib/variables/variable';
import { generateDecorationType } from '../../../lib/colorize-config';
...
describe('Test variables declaration Regex', () => {
  ...
  it('Should match (css variables)', function () {
    ...
    assert.equal(
      regex_exec('--varTwo       :', DECLARATION_REGEXP)[1],
      '--varTwo       ',
    );
  });
...
describe('Test variables use regexp', function () {
  ...
  it('Should match (css variables)', function () {
    ...
    assert.equal(regex_exec('var(--var--two)', REGEXP)[2], '--var--two');
    assert.equal(
      regex_exec('var(--var--two--three)', REGEXP)[2],
      '--var--two--three',
    );
    assert.equal(
      regex_exec('var(--var-two-three)', REGEXP)[2],
