...
import {
  REGEXP,
  DECLARATION_REGEXP,
} from '../../src/lib/variables/strategies/sass-strategy';
...
describe('Test variables use regexp', function () {
  ...
  it('Should match (sass variables)', function () {
    ...
    assert.equal(
      regex_exec('$var-two-tree', REGEXP)[1],
      '$var-two-tree',
    ...
    assert.equal(
      regex_exec('$normal-var', REGEXP)[1],
      '$normal-var',
      '"$normal-var" should match',
    );
    assert.equal(
      regex_exec('$with-number1', REGEXP)[1],
      '$with-number1',
      '"$with-number1" should match',
    );
    assert.equal(
      regex_exec('$with-letterA', REGEXP)[1],
      '$with-letterA',
      '"$with-letterA" should match',
    );
    assert.equal(
      regex_exec('$with-dash-1', REGEXP)[1],
      '$with-dash-1',
      '"$with-dash-1" should match',
    );
    assert.equal(
      regex_exec('$with-dash-A', REGEXP)[1],
      '$with-dash-A',
      '"$with-dash-A" should match',
    );
    assert.equal(
      regex_exec('$with-underscore_2', REGEXP)[1],
      '$with-underscore_2',
      '"$with-underscore_2" should match',
    );
    assert.equal(
      regex_exec('$with-underscore_B', REGEXP)[1],
      '$with-underscore_B',
      '"$with-underscore_B" should match',
    );
