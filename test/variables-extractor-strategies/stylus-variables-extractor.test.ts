...
describe('Test variables declaration Regex', () => {
  ...
  it('Should match (stylus variables)', function () {
    ...
    assert.equal(regex_exec('__=', DECLARATION_REGEXP)[1], '__', '"__" should match');
    assert.equal(regex_exec('--a=', DECLARATION_REGEXP)[1], '--a', '"--a==" should match');
    assert.equal(regex_exec('--a1=', DECLARATION_REGEXP)[1], '--a1', '"--a1=" should match');
    assert.equal(regex_exec('-a=', DECLARATION_REGEXP)[1], '-a', '"-a=" should match');
    assert.equal(regex_exec('_a=', DECLARATION_REGEXP)[1], '_a', '"_a=" should match');
    assert.equal(regex_exec('$a=', DECLARATION_REGEXP)[1], '$a', '"$a=" should match');
    assert.equal(regex_exec('$=', DECLARATION_REGEXP)[1], '$', '"$=" should match');
  });
  it('Should not match (stylus variables)', function () {
    assert.isNull(regex_exec('var', DECLARATION_REGEXP), '"var" is not a valid variable declaration');
    ...
    assert.isNull(regex_exec('--=', DECLARATION_REGEXP), '"--=" is not a valid variable declaration');
    assert.isNull(regex_exec('120=', DECLARATION_REGEXP), '"120=" is not a valid variable declaration');
    assert.isNull(regex_exec('@=', DECLARATION_REGEXP), '"@=" is not a valid variable declaration');
    assert.isNull(regex_exec('@a=', DECLARATION_REGEXP), '"@a=" is not a valid variable declaration');
    assert.isNull(regex_exec('1a=', DECLARATION_REGEXP), '"1a=" is not a valid variable declaration');
    // assert.isNull(regex_exec(' =', DECLARATION_REGEXP), '" =" is not a valid variable declaration'); // match (" ") but not the strategy get rid of this
  });
});
...
describe('Test variables use regexp', function() {
  ...
  it('Should match (stylus variables)', function () {
    ...
    assert.equal(regex_exec('var-two', REGEXP)[2], 'var-two', '"var-two" should match');
    assert.equal(regex_exec('var--two', REGEXP)[2], 'var--two', '"var--two" should match');
    assert.equal(regex_exec('var-one-two', REGEXP)[2], 'var-one-two', '"var-one-two" should match');
    assert.equal(regex_exec('var--one--two', REGEXP)[2], 'var--one--two', '"var--one--two" should match');
