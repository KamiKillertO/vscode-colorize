...
export const REGEXP_ONE = new RegExp(`(^|(?::|=)\\s*)((?:[\\-]*[$a-z_][\\-_\\d]*)+)(?!=)${EOL}`, 'i');
export const DECLARATION_REGEXP = new RegExp(`(?:(^(?:\\$|(?:[\\-_$]+[a-z\\d]+)|(?:[^\\d||\\-|@]+))(?:[_a-z\\d][\\-]*)*))\\s*=${EOL}`, 'gi');
...
VariablesExtractor.registerStrategy(new VariableStrategy('STYLUS', DECLARATION_REGEXP, REGEXP, REGEXP_ONE, RegexpExtractor));
